...
Each result is added to the archive via `NbxModel::addNewFile`, which delegates to `Fnx::Xml::addNewFile`. The extension parameter flows into the XML manifest's `extension` attribute and determines the on-disk content filename (`{uuid}{ext}`). The source file's stem becomes the display name. Imported files are opened after insertion, and the tree view expands to show the last imported file.