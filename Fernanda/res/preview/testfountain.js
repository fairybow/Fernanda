...
; (function () {
    ...
    var regex = {
        /* --- BEGIN for Fernanda --- */
        fernanda_annotation: /(<a id=(?:'|"))(preview-block-[0-9]+)((?:'|")><\/a>(?:\n)?)/gim,
        /* --- END for Fernanda --- */
    ...
    /* --- BEGIN for Fernanda --- */
    // tests
    var deannotate = function (script) {
        return script.replace(regex.fernanda_annotation, '');
    };
    /* --- END for Fernanda --- */

    var lexer = function (script) {
        return deannotate(script).replace(regex.boneyard, '\n$1\n') // adding the deannotate preserves line break syntax rendering...
    ...
    var tokenize = function (script) {
        ...
        while (i--) {
            ...
            /* --- BEGIN for Fernanda --- */
            if (match = line.match(regex.fernanda_annotation)) {
                //text = match[2]; // it should be match 2...
                //tokens.push({ type: 'fernanda_annotation', text: match[2] });
                //line = line.replace(regex.fernanda_annotation, ''); // works to remove annotations, but breaks page breaks? "==="
            }
            /* --- END for Fernanda --- */
    ...
    var parse = function (script, toks, callback) {
        ...
        while (i--) {
            ...
            switch (token.type) {
                ...
                /* --- BEGIN for Fernanda --- */
                //case 'fernanda_annotation': html.push('<a id=\"' + token.text + '\"></a>'); break;
                //case 'fernanda_annotation': html.push(token.text); break; // this works but the above doesn't???
                /* --- END for Fernanda --- */
