...
public:
    FontSelector(const QFont& initialFont, QWidget* parent = nullptr)
        : QWidget(parent), currentFont_(initialFont)
    {
        initialize_();
    }
...
private:
    ...
    QCheckBox* boldCheckBox_ = new QCheckBox(Tr::FontSelector::bold(), this); // Eventually, use Switch (maybe)
    QCheckBox* italicCheckBox_ = new QCheckBox(Tr::FontSelector::italic(), this); // ^
...
private:
    void initialize_()
    {
        ...
        sizeSlider_->setRange(Ini::EditorFont::PT_SIZE_MIN, Ini::EditorFont::PT_SIZE_MAX);
        ...
        // Connect
        connect(fontsBox_, &QComboBox::currentTextChanged, this, [&](const QString& text)
            {
        ...
        connect(sizeSlider_, &DisplaySlider::valueChanged, this, [&](int value)
            {
