...
#include "TabViewCloseButton.h"
#include "TabViewTabBar.h"
#include "TabViewUnderlay.h"
...
public:
    ...
    virtual ~TabView() override { COCO_TRACER; }
...
private:
    ...
    TabViewTabBar* tabBar_ = new TabViewTabBar(this);
    // Scroll right here, eventually
    TabViewButton* addButton_ = new TabViewButton(this);
    ...
    TabViewUnderlay* underlay_ = new TabViewUnderlay(this);
    ...
    void initialize_()
    {
        ...
        addButton_->setSvgPath(":/ui/TabViewButtonAdd.svg");
        ...
        (
            ...
            this,
            &TabView::onTabBarCurrentChanged_
        );
        ...
        (
            ...
            this,
            &TabView::onWidgetStackCurrentChanged_
        );
...
public:
    ...
    T removeTab(int index) { return qobject_cast<T>(removeTab(index)); }

    QWidgetList clear()
    {
        QWidgetList widgets{};
        while (!isEmpty()) widgets << removeTab(0);
        return widgets;
    }
...
private:
    int addOrInsertTab_(int index, QWidget* widget, const QString& tabText)
    {
        ...
        // If this was the first tab, emit the signal manually after everything is set up
        if (tabBar_->count() == 1)
...
public:
    ...
    {
        if (auto button = closeButtonAt_(index))
            button->setFlagged(flagged);
    }

    QVariant tabData(int index) const { return tabUserData_[widgetAt(index)]; }
    void setTabData(int index, const QVariant& data) { tabUserData_[widgetAt(index)] = data; }
...
private:
    ...
    QList<TabViewCloseButton*> closeButtons_{};
    ...
    void updateMouseHoverAfterLayoutChange_()
    {
        ...
        auto close_button_under_mouse = qobject_cast<TabViewCloseButton*>(widget_under_mouse);
    ...
    TabViewCloseButton* closeButtonAt_(int index) const
    {
        auto button = tabBar_->tabButton(index, QTabBar::RightSide);
        return qobject_cast<TabViewCloseButton*>(button);
    }

    void addCloseButtonAt_(int index)
    {
        auto close_button = new TabViewCloseButton(tabBar_);
        close_button->setFixedSize(CLOSE_BUTTON_SIZE_);
        close_button->setSvgSize(BUTTON_SVG_SIZE_);
        close_button->setSvgPath(":/ui/TabViewButtonClose.svg");
        close_button->setFlagSvgPath(":/ui/TabViewButtonEditedDot.svg");
...
private slots:
    ...
    void onWidgetStackCurrentChanged_(int index)
    {
        ...
        widgetStack_->count()
            ? mainStack_->setCurrentWidget(widgetStack_)
...
public:
    ...
    template <Coco::Concepts::QWidgetPointer T>
    T currentWidget() const { return qobject_cast<T>(currentWidget()); }
    ...
    template <Coco::Concepts::QWidgetPointer T>
    T widgetAt(int index) const { return qobject_cast<T>(widgetAt(index)); }
...
public:
    ...
    void setTabIcon(int index, const QIcon& icon) { tabBar_->setTabIcon(index, icon); }

    QString tabToolTip(int index) const { return tabBar_->tabToolTip(index); }
    void setTabToolTip(int index, const QString& tip) { tabBar_->setTabToolTip(index, tip); }
    ...
    void setUsesScrollButtons(bool useButtons) { tabBar_->setUsesScrollButtons(useButtons); }

    bool isTabEnabled(int index) const { return tabBar_->isTabEnabled(index); }
    void setTabEnabled(int index, bool enabled) { tabBar_->setTabEnabled(index, enabled); }

    QString tabText(int index) const { return tabBar_->tabText(index); }
    void setTabText(int index, const QString& text) { tabBar_->setTabText(index, text); }
...
private:
    void initializeTabBarPropagatedSignals_()
    {
        ...
        (
            ...
            this,
            [&](int index) { emit tabBarClicked(index); }
        );
        ...
        (
            ...
            this,
            [&](int index) { emit tabBarDoubleClicked(index); }
        );
        ...
        (
            ...
            &QTabBar::tabMoved,
            this,
            [&](int from, int to) { emit tabMoved(from, to); }
        );
    }
    ...
    // - Icon for another Ecosystem should be "no" if possible...

public:
    using DragValidator = std::function<bool(TabView* source, TabView* destination)>;

    struct Location
    {
        QPointer<TabView> tabView = nullptr;
        int index = -1;
        bool isValid() const noexcept { return tabView && index > -1; }
    };
    ...
    void setDragValidator(const DragValidator& validator) { dragValidator_ = validator; }

    template <typename ClassT>
    void setDragValidator(ClassT* object, bool (ClassT::* method)(TabView*, TabView*))
    {
        dragValidator_ = [object, method](TabView* source, TabView* destination)
            {
        ...
        if (tabsDraggable_ && watched == tabBar_)
        {
            ...
            else if (event->type() == QEvent::MouseMove)
            {
                ...
                if (mouse_event->buttons() & Qt::LeftButton)
                {
                    ...
                    if (qAbs(delta.y()) >= QApplication::startDragDistance() * 1.5)
                    {
...
signals:
    ...
    void tabDraggedToDesktop(TabView* source, const QPoint& dropPos, const TabSpec& tabSpec);

protected:
    virtual void dragEnterEvent(QDragEnterEvent* event) override
    {
        event->mimeData()->hasFormat(MIME_TYPE_)
    ...
    virtual void dropEvent(QDropEvent* event) override
    {
        ...
        emit tabDragged({ tab_assembly.origin, tab_assembly.originIndex }, { this, new_index });
        event->acceptProposedAction();
    }
...
private:
    ...
    void startDrag_(int index)
    {
        ...
        drag->setHotSpot(drag_context.tabSpec.relPos(pixmap.width(), pixmap.height()));

        // Works to hide the "no" sign on valid desktop drags, but not having a cursor at all is bad
        ...
        case Qt::DropAction::IgnoreAction:
            if (isDesktopDrop_())
            {
                emit tabDraggedToDesktop(this, QCursor::pos(), drag_context.tabSpec);
                break;
            }
        ...
        case Qt::DropAction::TargetMoveAction:
            ...
            // Success state (handled by target TabView)
        case Qt::DropAction::MoveAction: break;
        }
    }
    ...
    QByteArray serialize_(const TabDragContext_& dragContext)
    {
        ...
        data_stream << quintptr(dragContext.origin)
            << dragContext.originIndex
            << quintptr(dragContext.tabSpec.widget)
            << dragContext.tabSpec.userData
            << dragContext.tabSpec.text
    ...
    TabDragContext_ deserialize_(QByteArray& data)
    {
        ...
        return
        {
            reinterpret_cast<TabView*>(origin),
            origin_index,
    ...
    QPixmap dragPixmap_(const QString& tabText) const
    {
        ...
        QLinearGradient bezel_border_gradient(main_rect.topLeft(), main_rect.bottomRight());
        ...
        auto content_rect = main_rect.adjusted(bezel_width, bezel_width, -bezel_width, -bezel_width);

        // Content gradient
        QLinearGradient content_gradient(content_rect.topLeft(), content_rect.bottomLeft());
        ...
        content_gradient.setColorAt(1.0, QColor(15, 15, 15, 180));  // Darkest at bottom

        QPainterPath content_path{};
        content_path.addRoundedRect(content_rect, corner_radius - 1, corner_radius - 1);
        ...
        static constexpr auto color = QColor(100, 100, 100, 100); // was 85, 85, 85, 80

        QLinearGradient highlight_gradient(content_rect.topLeft(), QPoint(content_rect.left(), content_rect.top() + gradient_height));
        highlight_gradient.setColorAt(0.0, color);
        highlight_gradient.setColorAt(1.0, Qt::transparent);

        QPainterPath highlight_path{};
        auto highlight_rect = content_rect.adjusted(inset, inset, -inset, -content_rect.height() + total_height);
        highlight_path.addRoundedRect(highlight_rect, corner_radius - radius_reduction, corner_radius - radius_reduction);
        ...
        auto elided_text = font_metrics.elidedText(tabText, Qt::ElideRight, text_rect.width());
