...
template<typename T>
concept QMenuOrStatusBar = Coco::Concepts::DerivedPointer<QMenuBar, T> || Coco::Concepts::DerivedPointer<QStatusBar, T>;
...
public:
    enum Position { Top, BelowMenuBar, AboveStatusBar, Bottom };
    enum Color { Green, Pastel, Red };

    explicit ColorBar(QMainWindow* parentWindow = nullptr)
        : QWidget(parentWindow), window_(parentWindow)
    {
        initialize_();
    }
    ...
    void setPlacement(Position position) { position_ = position; }
    ...
    virtual bool eventFilter(QObject* watched, QEvent* event) override
    {
        if (watched == window_)
        {
            ...
            case QEvent::Resize: updateGeometry_(); break;
            default: break;
            }
        }
    ...
    virtual void paintEvent(QPaintEvent* event) override
    {
        ...
        auto progress = (currentProgress_ - MIN_RANGE_) / (MAX_RANGE_ - MIN_RANGE_);

        // Calculate the visible width based on progress
        auto visible_width = static_cast<int>(rect.width() * progress);

        // Create the gradient using the stored color
        auto gradient = gradientFor_(currentColor_);

        // Only draw the portion that should be visible
...
private:
    ...
    template <QMenuOrStatusBar T>
    T getVisibleBar() const
    {
